...
class ResourceListViewController: UITableViewController {
	...
	override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		if let allergyInstance = resource as? AllergyIntolerance {
			if let allergyName: String = allergyInstance.substance?.coding?.first?.display {
				cell.textLabel?.text = allergyName
			} else {
				cell.textLabel?.text = "Allergy"
			}
			
			if let allergyReaction: String = allergyInstance.reaction?.first?.manifestation?.first?.text {
			...
			if let conditionName: String = conditionInstance.code?.coding?.first?.display {
			...
			if let documentName: String = documentInstance.type?.coding?.first?.display {
				cell.textLabel?.text = documentName
			} else if let documentName: String = documentInstance.description_fhir {
			...
			if let immunizationName: String = immunizationInstance.vaccineCode?.coding?.first?.display {
				cell.textLabel?.text = immunizationName
			} else if let immunizationName: String = immunizationInstance.vaccineCode?.text {
				cell.textLabel?.text = immunizationName
			} else {
		...
		else if let medicationOrderInstance = resource as? MedicationOrder {
			if let medName: String = medicationOrderInstance.medicationCodeableConcept?.coding?.first?.display {
				cell.textLabel?.text = medName
			} else if let medName: String = medicationOrderInstance.medicationReference?.display { // For Epic
				cell.textLabel?.text = medName
			} else {
				cell.textLabel?.text = "Medication"
			}
			
			if let medStatus: String = medicationOrderInstance.status {
			...
			if let observationValue: String = observationInstance.valueQuantity?.value?.stringValue {
				cell.textLabel?.text = observationValue + " " + (observationInstance.valueQuantity?.unit)!
			} else if let observationValue: String = observationInstance.valueString {
				cell.textLabel?.text = observationValue
			} else {
				cell.textLabel?.text = "Value Not Available"
			}
			
			if let observationName: String = observationInstance.code?.coding?.first?.display {
			...
			if let procedureName: String = procedureInstance.code?.coding?.first?.display {
